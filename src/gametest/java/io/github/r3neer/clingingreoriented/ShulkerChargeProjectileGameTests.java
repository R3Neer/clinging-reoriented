...
public final class ShulkerChargeProjectileGameTests {
    ...
    @GameTest(padding=40) public void targetBlockReceivesRealProjectileSignal(GameTestHelper h){
        BlockPos target=new BlockPos(16,8,8);h.setBlock(target,Blocks.TARGET);Vec3 origin=new Vec3(5.5,8.5,8.5);Vec3 intent=Vec3.atCenterOf(target).subtract(origin);
        var bullet=charge(h,origin,intent);h.assertTrue(((ShulkerChargeProjectile)(Object)bullet).clinging$targetBlock()!=null,"direct Target Block locked");
    ...
    @GameTest(padding=48) public void offsetTargetBlockRequiresOrthogonalRouteAndStillHits(GameTestHelper h){
        BlockPos target=new BlockPos(18,8,13);h.setBlock(target,Blocks.TARGET);Vec3 origin=new Vec3(5.5,8.5,5.5);Vec3 intent=Vec3.atCenterOf(target).subtract(origin);
    ...
    @GameTest(padding=40) public void entityImpactKeepsVanillaDamageAndLevitation(GameTestHelper h){
        var cow=h.spawn(EntityTypes.COW,new BlockPos(15,8,8));cow.setNoAi(true);float before=cow.getHealth();Vec3 origin=new Vec3(5.5,8.8,8.5);var bullet=charge(h,origin,cow.getBoundingBox().getCenter().subtract(h.absoluteVec(origin)));
    ...
    @GameTest(padding=30) public void blockImpactAndPlainDiscardReturnNoItem(GameTestHelper h){
        BlockPos wall=new BlockPos(11,8,8);h.setBlock(wall,Blocks.STONE);Vec3 origin=new Vec3(7.5,8.5,8.5);var bullet=charge(h,origin,new Vec3(1,0,0));
    ...
    @GameTest(padding=50) public void shieldedPlayerNeutralizesChargeWithoutDrop(GameTestHelper h){
        var player=h.makeMockServerPlayerInLevel();player.snapTo(h.absoluteVec(new Vec3(16,8,8)));player.setItemInHand(InteractionHand.OFF_HAND,new ItemStack(Items.SHIELD));player.startUsingItem(InteractionHand.OFF_HAND);
    ...
    @GameTest(padding=60) public void launchedChargePreservesVanillaShulkerDuplication(GameTestHelper h){
        for(int x=2;x<=28;x++)for(int z=2;z<=28;z++)h.setBlock(new BlockPos(x,6,z),Blocks.STONE);
