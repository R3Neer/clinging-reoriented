    ...
    @Override public void runTest(ClientGameTestContext context){
        try(var world=context.worldBuilder().create()){
            ...
            world.getServer().runOnServer(server->{
                ...
                var frame=new ItemFrame(level,framePos,Direction.NORTH);frame.setInvisible(true);frame.setItem(new ItemStack(ShulkerCharges.ITEM),false);
                if(!level.addFreshEntity(frame))throw new AssertionError("Could not add Shulker Charge fixed-model evidence frame");
            });
            context.waitTicks(3);
            context.runOnClient(mc->{
                if(mc.level==null)throw new AssertionError("Client level missing during fixed model evidence");
                boolean found=false;
                for(var entity:mc.level.entitiesForRendering())if(entity instanceof ItemFrame frame&&frame.getItem().is(ShulkerCharges.ITEM)){found=true;break;}
                if(!found)throw new AssertionError("Shulker Charge fixed-model evidence frame is not present on client");
            });
            context.takeScreenshot("shulker-charge-fixed-3d");
        }
    }
}
