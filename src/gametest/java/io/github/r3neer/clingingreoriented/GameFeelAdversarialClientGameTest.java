...
public final class GameFeelAdversarialClientGameTest implements FabricClientGameTest {
    ...
    @Override public void runTest(ClientGameTestContext context){
        ...
        try(var world=context.worldBuilder().create()){
            ...
            // CameraType updates options immediately but mainCamera is rebuilt on later render ticks.
            // Let that settle before freezing the baseline; FA/EMF can expose the same-call race.
            context.waitTicks(3);
            ...
            context.runOnClient(mc->{advance(mc,1,new Vec3(0,-.20,0));assertVec(new Vec3(0,-1,0),BodyOrientation.bodyUp(body(mc)),"physical gravity turn rotated body before velocity changed");assertVec(camera.get(),cameraForward(mc),"physical gravity turn moved camera during Gravity Fall");advance(mc,1,new Vec3(0,0,-.20));assertDirectionWithin(new Vec3(0,0,-1),BodyOrientation.bodyUp(body(mc)),AERO_BODY_MAX_OFFSET_RADIANS,"body stopped being velocity-owned after NORTH reorientation");assertVec(camera.get(),cameraForward(mc),"velocity-owned body turn fed back into camera");});
            context.takeScreenshot("s05-gravity-fall-velocity-not-gravity");
            AtomicReference<Quaternionf> landingStart=new AtomicReference<>();context.runOnClient(mc->{landingStart.set(new Quaternionf(VisualTransitions.current(mc.player)));VisualTransitions.land(mc.player,Direction.NORTH,1,90_000L);});context.waitTicks(1);
            context.runOnClient(mc->{Quaternionf partial=new Quaternionf(VisualTransitions.current(mc.player));if(equivalent(partial,landingStart.get()))throw new AssertionError("landing cancel holdout never left retained start frame");if(equivalent(partial,RotationUtil.getEntityRotationQuaternion(Direction.NORTH)))throw new AssertionError("landing cancel holdout reached endpoint before cancellation");cancelledLanding.set(partial);VisualTransitions.cancel(mc.player,true,90_001L);if(!VisualTransitions.holding(mc.player))throw new AssertionError("landing cancellation did not retain the current partial frame");assertQuat(cancelledLanding.get(),VisualTransitions.current(mc.player),"cancel snapped away from current partial landing frame");});context.waitTicks(3);context.runOnClient(mc->assertQuat(cancelledLanding.get(),VisualTransitions.current(mc.player),"cancelled landing frame drifted instead of holding"));context.takeScreenshot("s05-cancelled-landing-holds-partial");
    ...
    private static void assertDirectionWithin(Vec3 expected,Vec3 actual,double maxRadians,String label){Vec3 a=expected.normalize(),b=actual.normalize();double dot=Math.max(-1.0D,Math.min(1.0D,a.dot(b)));double angle=Math.acos(dot);if(!Double.isFinite(angle)||angle>maxRadians)throw new AssertionError(label+": angleDeg="+Math.toDegrees(angle)+" expected="+a+" actual="+b);}
