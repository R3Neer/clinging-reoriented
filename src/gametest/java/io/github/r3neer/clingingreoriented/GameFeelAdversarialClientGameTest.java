    ...
    @Override public void runTest(ClientGameTestContext context){
        ...
        try(var world=context.worldBuilder().create()){
            ...
            context.runOnClient(mc->{
                ...
                VisualTransitions.land(mc.player,Direction.NORTH,1,(float)LandingTiming.PRESENTATION_TICKS,90_000L);
            });
            context.waitTicks(1);
            context.runOnClient(mc->{
                ...
                if(!VisualTransitions.owns(mc.player))throw new AssertionError("landing cancellation released presentation instead of recovering the retained flight frame");
                assertQuat(cancelledLanding.get(),VisualTransitions.current(mc.player),"cancel jumped away from the current partial landing frame");
            });
            context.waitTicks(LandingTiming.RECOVERY_TICKS+2);
            context.runOnClient(mc->{
                if(!VisualTransitions.holding(mc.player))throw new AssertionError("landing recovery did not settle back into HOLD");
                assertQuat(landingStart.get(),VisualTransitions.current(mc.player),"cancelled landing did not recover the retained free-flight frame");
            });
            context.takeScreenshot("s05-cancelled-landing-recovers-hold");
