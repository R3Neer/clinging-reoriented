...
public final class GravityChargeProjectileGameTests {
        ...
        ServerEntityEvents.ALLOW_LOAD.register((entity,level,spawnReason,isLoadedFromDisk)->{
            if(entity.getType()==EntityTypes.SHULKER){
                for(ShulkerSpawnWatch watch:SHULKER_SPAWN_WATCHES){
                    if(watch.level==level&&watch.area.contains(entity.position())){
                        watch.allowed.incrementAndGet();
                        watch.lastReason.set(spawnReason);
                    }
                }
            }
            return true;
    ...
    private static final class ShulkerSpawnWatch {
        ...
        private final AtomicReference<EntitySpawnReason> lastReason=new AtomicReference<>();
        ...
        private EntitySpawnReason lastReason(){return lastReason.get();}
    ...
    private static ShulkerBullet chargeAbsolute(GameTestHelper h,Vec3 absolute,Vec3 intent){
        ...
        h.assertTrue(h.getLevel().addFreshEntity(bullet),"Gravity Charge fixture must be admitted to the server entity manager");
        return bullet;
    }
    private static long drops(GameTestHelper h,Vec3 relative){Vec3 p=h.absoluteVec(relative);return h.getLevel().getEntitiesOfClass(ItemEntity.class,new AABB(p.x-4,p.y-4,p.z-4,p.x+4,p.y+4,p.z+4),e->e.getItem().is(GravityCharges.ITEM)).size();}
    ...
    @GameTest(maxTicks=220,padding=48) public void targetBlockReceivesRealProjectileSignal(GameTestHelper h){
        ...
        Vec3 center=Vec3.atCenterOf(target),origin=center.add(-6,0,0);keepSimulated(h,origin,center);var bullet=chargeAbsolute(h,origin,center.subtract(origin));var state=(GravityChargeProjectile)(Object)bullet;
        ...
        h.startSequence().thenExecuteAfter(1,()->h.assertTrue(bullet.tickCount>0,"direct Target Block Gravity Charge must enter entity ticking")).thenWaitUntil(()->h.assertTrue(level.getBlockState(target).getValue(BlockStateProperties.POWER)>0,"Target Block must be powered by physical projectile impact; ticks="+bullet.tickCount+" alive="+bullet.isAlive()+" pos="+bullet.position()+" vel="+bullet.getDeltaMovement()+" target="+target)).thenExecute(()->h.assertTrue(!bullet.isAlive(),"impact destroys Gravity Charge through vanilla ShulkerBullet onHit")).thenSucceed();
    }
    ...
    @GameTest(maxTicks=260,padding=64) public void offsetTargetBlockRequiresOrthogonalRouteAndStillHits(GameTestHelper h){
        ...
        Vec3 center=Vec3.atCenterOf(target),origin=center.add(-10,0,-6);keepSimulated(h,origin,center);var bullet=chargeAbsolute(h,origin,center.subtract(origin));var state=(GravityChargeProjectile)(Object)bullet;
        h.assertTrue(target.equals(state.clinging$targetBlock()),"offset Target Block fixture locks exact absolute target");
        h.startSequence().thenExecuteAfter(1,()->{h.assertTrue(bullet.tickCount>0,"offset Target Block Gravity Charge must enter entity ticking");Vec3 movement=bullet.getDeltaMovement();int axes=(Math.abs(movement.x)>1.0E-4?1:0)+(Math.abs(movement.y)>1.0E-4?1:0)+(Math.abs(movement.z)>1.0E-4?1:0);h.assertTrue(axes==1,"active Shulker route segment is cardinal after first steering tick");}).thenWaitUntil(()->h.assertTrue(level.getBlockState(target).getValue(BlockStateProperties.POWER)>0,"multi-axis Target Block route reaches real block; ticks="+bullet.tickCount+" alive="+bullet.isAlive()+" pos="+bullet.position()+" vel="+bullet.getDeltaMovement()+" target="+target)).thenSucceed();
    }
    ...
    @GameTest(maxTicks=180,padding=48) public void entityImpactKeepsVanillaDamageAndLevitation(GameTestHelper h){
        ...
        h.assertTrue(((GravityChargeProjectile)(Object)bullet).clinging$targetEntity()==cow,"damage fixture owns cow target before flight");
        h.startSequence().thenExecuteAfter(1,()->h.assertTrue(bullet.tickCount>0,"entity-impact Gravity Charge must enter entity ticking")).thenWaitUntil(()->{h.assertTrue(cow.getHealth()<before,"Gravity Charge damages stationary entity through vanilla hit path; ticks="+bullet.tickCount+" alive="+bullet.isAlive()+" pos="+bullet.position()+" vel="+bullet.getDeltaMovement());h.assertTrue(cow.hasEffect(MobEffects.LEVITATION),"Gravity Charge applies vanilla Levitation");}).thenExecute(()->h.assertTrue(Math.abs((before-cow.getHealth())-4.0F)<0.01F,"unarmored target receives vanilla 4 damage")).thenExecute(()->h.assertTrue(!bullet.isAlive(),"entity impact consumes projectile")).thenSucceed();
    }
    ...
    @GameTest(padding=40) public void launchedChargeCanBeRecapturedExactlyOnceByMeleeOrArrow(GameTestHelper h){
        ...
        var melee=charge(h,a,new Vec3(0,1,0));melee.hurtServer(h.getLevel(),h.getLevel().damageSources().playerAttack(player),1.0F);h.assertTrue(drops(h,a)==1,"melee recaptures launched Gravity Charge");melee.hurtServer(h.getLevel(),h.getLevel().damageSources().playerAttack(player),1.0F);h.assertTrue(drops(h,a)==1,"melee race cannot duplicate item");
        var arrowEntity=new Arrow(h.getLevel(),player,new ItemStack(Items.ARROW),null);var ranged=charge(h,b,new Vec3(0,1,0));ranged.hurtServer(h.getLevel(),h.getLevel().damageSources().arrow(arrowEntity,player),1.0F);h.assertTrue(drops(h,b)==1,"arrow recaptures launched Gravity Charge");h.succeed();
    }
    ...
    @GameTest(maxTicks=100,padding=80) public void blockImpactAndPlainDiscardReturnNoItem(GameTestHelper h){
        ...
        Vec3 center=Vec3.atCenterOf(wall),origin=center.add(-2,0,0);keepSimulated(h,origin,center);var bullet=chargeAbsolute(h,origin,center.subtract(origin));var state=(GravityChargeProjectile)(Object)bullet;
        ...
        h.startSequence().thenExecuteAfter(1,()->h.assertTrue(bullet.tickCount>0,"free-flight Gravity Charge must enter entity ticking")).thenWaitUntil(()->h.assertTrue(!bullet.isAlive(),"Gravity Charge reaches ordinary block and is consumed; ticks="+bullet.tickCount+" start="+origin+" wall="+wall+" pos="+bullet.position()+" vel="+bullet.getDeltaMovement())).thenExecute(()->h.assertTrue(level.getEntitiesOfClass(ItemEntity.class,new AABB(Vec3.atCenterOf(wall),Vec3.atCenterOf(wall)).inflate(5),e->e.getItem().is(GravityCharges.ITEM)).isEmpty(),"ordinary block impact never returns Gravity Charge item")).thenExecute(()->{var discarded=chargeAbsolute(h,center.add(-2,4,0),new Vec3(0,1,0));discarded.discard();h.assertTrue(!discarded.isAlive(),"plain discard removes Gravity Charge");}).thenSucceed();
    }
    ...
    @GameTest(maxTicks=180,padding=56) public void shieldedPlayerNeutralizesChargeWithoutDrop(GameTestHelper h){
        ...
        h.assertTrue(((GravityChargeProjectile)(Object)bullet).clinging$targetEntity()==player,"shield fixture owns the intended stationary player lock");
        h.startSequence().thenExecuteAfter(1,()->h.assertTrue(bullet.tickCount>0,"shield Gravity Charge must enter entity ticking")).thenWaitUntil(()->h.assertTrue(!bullet.isAlive(),"shield encounter consumes Gravity Charge projectile; ticks="+bullet.tickCount+" pos="+bullet.position()+" player="+player.position())).thenExecute(()->{h.assertTrue(player.getHealth()==before,"shield blocks Gravity Charge damage");}).thenSucceed();
    }
    ...
    @GameTest(maxTicks=180,padding=60) public void launchedChargePreservesVanillaShulkerDuplication(GameTestHelper h){
        ...
        Vec3 oldPosition=shulker.position();AABB oldDensityArea=shulker.getBoundingBox().inflate(8.0D);var offspring=new ShulkerSpawnWatch(level,oldDensityArea);
        ...
        h.assertTrue(bullet.getType()==EntityTypes.SHULKER_BULLET,"launched Gravity Charge keeps exact vanilla type required by Shulker duplication");h.assertTrue(((GravityChargeProjectile)(Object)bullet).clinging$targetEntity()==shulker,"duplication fixture owns shulker target");
        h.startSequence()
            .thenExecuteAfter(1,()->{h.assertTrue(bullet.tickCount>0,"duplication Gravity Charge must enter entity ticking");h.assertTrue(shulker.position().distanceToSqr(oldPosition)<0.01D,"fixture Shulker must remain physically anchored until impact; old="+oldPosition+" current="+shulker.position());})
            .thenWaitUntil(()->h.assertTrue(!bullet.isAlive(),"Gravity Charge physically reaches and is consumed by shulker interaction; ticks="+bullet.tickCount+" pos="+bullet.position()+" vel="+bullet.getDeltaMovement()))
            ...
            .thenExecute(()->{int count=level.getEntities(EntityTypes.SHULKER,oldDensityArea,Shulker::isAlive).size();h.assertTrue(count<=1,"vanilla duplication density gate must have zero failure probability in fixture; count="+count+" area="+oldDensityArea);})
            .thenWaitUntil(()->h.assertTrue(offspring.allowed()>=1,"vanilla hitByShulkerBullet must submit a new Shulker to ALLOW_LOAD; allowed="+offspring.allowed()+" loaded="+offspring.loaded()+" reason="+offspring.lastReason()+" parent="+shulker.position()+" old="+oldPosition+" health="+shulker.getHealth()))
            .thenWaitUntil(()->h.assertTrue(offspring.loaded()>=1,"new vanilla Shulker must enter ServerLevel tracking; allowed="+offspring.allowed()+" loaded="+offspring.loaded()+" reason="+offspring.lastReason()+" parent="+shulker.position()+" old="+oldPosition+" health="+shulker.getHealth()))
