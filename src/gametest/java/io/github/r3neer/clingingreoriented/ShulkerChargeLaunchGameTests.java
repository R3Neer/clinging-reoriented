...
public final class ShulkerChargeLaunchGameTests {
    ...
    @GameTest(padding=40) public void manualUseConsumesOneAndSpawnsOwnedVanillaBullet(GameTestHelper h){
        ...
        h.assertTrue(bullet.getType()==EntityTypes.SHULKER_BULLET,"launched Charge keeps exact vanilla entity type");h.assertTrue(bullet.getOwner()==player,"player attribution retained");h.assertTrue(bullet.isNoGravity(),"targetless Charge does not fall");h.succeed();
    }
    ...
    @GameTest public void dispenserFactoryKeepsFacingIntentAndExactEntityType(GameTestHelper h){
        ...
        var projectile=((ShulkerChargeItem)ShulkerCharges.ITEM).asProjectile(h.getLevel(),p,new ItemStack(ShulkerCharges.ITEM),Direction.WEST);
        ...
        h.assertTrue(state.clinging$isLaunchedCharge()&&state.clinging$intent().distanceToSqr(new Vec3(-1,0,0))<1.0E-12,"facing becomes exact original intent");h.assertTrue(bullet.isNoGravity(),"dispenser Charge is no-gravity");h.succeed();
    }
    ...
    @GameTest public void launchedStateAndIntentSurviveSaveLoad(GameTestHelper h){
        ...
        var state=duck(restored);h.assertTrue(state.clinging$isLaunchedCharge(),"save/load keeps Charge identity");h.assertTrue(state.clinging$intent().distanceToSqr(ShulkerChargeTargeting.safeIntent(new Vec3(1,0.2,0)))<1.0E-12,"save/load keeps original aim intent");h.assertTrue(restored.isNoGravity(),"restored Charge remains no-gravity");h.succeed();
    }
}
