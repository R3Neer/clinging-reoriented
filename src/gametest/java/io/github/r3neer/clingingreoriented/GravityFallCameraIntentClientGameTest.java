...
/** CAM-S01/S05: screen-space handedness and camera-mode continuity through full-sphere poles. */
public final class GravityFallCameraIntentClientGameTest implements FabricClientGameTest {
    ...
    private static final double FRAME_EPS=3.0E-3D;
    ...
    @Override public void runTest(ClientGameTestContext context){
        ...
        AtomicReference<Vec3> modeForward=new AtomicReference<>();
        AtomicReference<Vec3> modeUp=new AtomicReference<>();
        try(var world=context.worldBuilder().create()){
            ...
            // CAM-S05: changing camera mode must not reinterpret the same full-sphere frame.
            context.runOnClient(mc->{modeForward.set(cameraForward(mc));modeUp.set(cameraUp(mc));mc.options.setCameraType(CameraType.FIRST_PERSON);});
            context.waitTicks(3);
            context.runOnClient(mc->{assertVec(modeForward.get(),cameraForward(mc),"third -> first changed forward");assertVec(modeUp.get(),cameraUp(mc),"third -> first changed screen-up");mc.options.setCameraType(CameraType.THIRD_PERSON_BACK);});
            context.waitTicks(3);
            context.runOnClient(mc->{assertVec(modeForward.get(),cameraForward(mc),"first -> third changed forward");assertVec(modeUp.get(),cameraUp(mc),"first -> third changed screen-up");});
            context.takeScreenshot("cam-s05-first-third-continuity");
    ...
    private static void activate(Minecraft mc,AtomicLong sequence){
        ...
        for(int i=0;i<8;i++){mc.player.setDeltaMovement(new Vec3(0,-.2,0));GravityFallVisuals.tick(mc);}mc.player.setDeltaMovement(Vec3.ZERO);
        if(!GravityFallVisuals.active(mc.player))throw new AssertionError("CAM-S01 could not activate Gravity Fall");
    }

    private static void setLook(Minecraft mc,float yaw,float pitch){mc.player.setYRot(yaw);mc.player.yRotO=yaw;mc.player.setXRot(pitch);mc.player.xRotO=pitch;GravityFallLookState.reseed(mc.player);}
    ...
    private static void assertVec(Vec3 expected,Vec3 actual,String label){if(expected.distanceTo(actual)>FRAME_EPS)throw new AssertionError(label+": expected="+expected+" actual="+actual);}
