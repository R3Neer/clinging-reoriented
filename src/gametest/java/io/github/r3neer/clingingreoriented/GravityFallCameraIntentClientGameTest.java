...
/** CAM-S01 red fixtures: screen-space handedness must survive full-sphere pole crossings. */
public final class GravityFallCameraIntentClientGameTest implements FabricClientGameTest {
    ...
    @Override public void runTest(ClientGameTestContext context){
        ...
        try(var world=context.worldBuilder().create()){
            world.getServer().runOnServer(server->{
                ...
                for(var pos:BlockPos.betweenClosed(new BlockPos(-12,75,-12),new BlockPos(12,105,12)))
            ...
            context.runOnClient(mc->{
                ...
                activate(mc,sequence);
                setLook(mc,0.0F,80.0F);
            });
            context.waitTicks(3);

            // Canonical side of the lower pole: establish the screen-space sign of positive horizontal input.
            context.runOnClient(mc->sample.set(beginHorizontalSample(mc)));
            context.waitTicks(2);
            context.runOnClient(mc->canonicalDown.set(horizontalResponse(mc,sample.get(),"first-person +80 baseline")));

            // Same input just beyond the pole must keep the same screen-space sign.
            context.runOnClient(mc->setLook(mc,0.0F,100.0F));
            context.waitTicks(2);
            context.runOnClient(mc->sample.set(beginHorizontalSample(mc)));
            context.waitTicks(2);
            ...
            // Repeat across the upper pole. A sign patch for only positive pitch is not sufficient.
            context.runOnClient(mc->setLook(mc,0.0F,-80.0F));
            context.waitTicks(2);
            context.runOnClient(mc->sample.set(beginHorizontalSample(mc)));
            context.waitTicks(2);
            ...
            // Third person: the camera boom/orbit must preserve the same handedness through the pole.
            context.runOnClient(mc->{mc.options.setCameraType(CameraType.THIRD_PERSON_BACK);setLook(mc,0.0F,80.0F);});
            context.waitTicks(3);
            context.runOnClient(mc->sample.set(beginOrbitSample(mc)));
            context.waitTicks(3);
            ...
            context.runOnClient(mc->{GravityFallVisuals.clear();mc.options.setCameraType(CameraType.FIRST_PERSON);setLook(mc,0,0);});
        }
    }
    ...
    private static Sample beginHorizontalSample(Minecraft mc){
        Sample before=new Sample(cameraForward(mc),cameraRight(mc),mc.gameRenderer.mainCamera().position());
        mc.player.turn(40.0D,0.0D);
        return before;
    }

    private static Sample beginOrbitSample(Minecraft mc){
        Sample before=new Sample(cameraForward(mc),cameraRight(mc),mc.gameRenderer.mainCamera().position());
        mc.player.turn(40.0D,0.0D);
        return before;
    }

    private static double horizontalResponse(Minecraft mc,Sample before,String label){
        double response=cameraForward(mc).subtract(before.forward()).dot(before.right());
        if(Math.abs(response)<RESPONSE_EPS)throw new AssertionError(label+" produced no measurable horizontal response");
        return response;
    }

    private static double orbitResponse(Minecraft mc,Sample before,String label){
        double response=mc.gameRenderer.mainCamera().position().subtract(before.position()).dot(before.right());
        if(Math.abs(response)<RESPONSE_EPS)throw new AssertionError(label+" produced no measurable orbit response");
        return response;
    }

    private static void assertSameSign(double canonical,double actual,String label){
        if(Math.signum(canonical)!=Math.signum(actual))
            throw new AssertionError(label+" inverted screen-space handedness: canonical="+canonical+" actual="+actual);
    }
