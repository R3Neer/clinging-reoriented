...
public final class VisualTransitions {
    ...
    private enum Mode { HOLD, LAND, SNAP, RECOVER_HOLD }
    private record Active(Entity entity,Direction target,GravityTransition.TurnKind kind,long sequence,Quaternionf startVisual,Quaternionf endVisual,
    ...
    public static void hold(Entity entity,Direction target,float yawDelta,long sequence){
        ...
        put(animation,new Active(entity,target,null,sequence,held,null,now,0L,actual==target,Mode.HOLD,0L));
    ...
    public static void land(Entity entity,Direction target,int kindId,float etaTicks,long sequence){
        ...
        put(animation,new Active(entity,target,kind,sequence,current,null,now,seen?now:0L,seen,Mode.LAND,duration));
        latestSequence=sequence;
    }

    /** Cancel a landing: recover to its retained pre-landing frame, or release ownership completely. */
    public static void cancel(Entity entity,boolean recoverHeldFrame,long sequence){
        if(sequence<=latestSequence||entity==null||sequence<0)return;
        GravityRotationAnimation animation=animation(entity);if(animation==null)return;
        Active previous; synchronized(ACTIVE){previous=ACTIVE.get(animation);}
        long now=System.nanoTime();Direction actual=GravityDirectionUtil.getGravityDirection(entity);
        Quaternionf current=animation.getRotation(actual,now);
        animation.forceSet(actual,now);
        if(recoverHeldFrame&&previous!=null&&previous.mode()==Mode.LAND){
            Quaternionf held=new Quaternionf(previous.startVisual());
            put(animation,new Active(entity,actual,null,sequence,current,held,now,now,true,Mode.RECOVER_HOLD,LandingTiming.RECOVERY_NANOS));
        }else if(recoverHeldFrame)put(animation,new Active(entity,actual,null,sequence,current,null,now,0L,true,Mode.HOLD,0L));
    ...
    private static boolean enrollSnap(Entity entity,Direction target,float yawDelta,int kindId,long sequence){
        ...
        GravityTransition.applyYawGauge(entity,yawDelta);
        put(animation,new Active(entity,target,kind,sequence,start,null,now,0L,false,Mode.SNAP,kind.durationNanos()));
        return true;
    }
    ...
    public static Quaternionf override(GravityRotationAnimation animation,Direction actual,long now){
        ...
        if(active.mode()==Mode.RECOVER_HOLD)return recoverOverride(animation,active,actual,now);
    ...
    private static Quaternionf holdOverride(GravityRotationAnimation animation,Active active,Direction actual,long now){
        ...
        if(!active.targetSeen()){
            if(actual==active.target()){
                ...
                active=new Active(active.entity(),active.target(),null,active.sequence(),active.startVisual(),null,active.receivedNanos(),0L,true,Mode.HOLD,0L);
    ...
    private static Quaternionf recoverOverride(GravityRotationAnimation animation,Active active,Direction actual,long now){
        if(actual!=active.target()||active.endVisual()==null){clear(animation);return null;}
        ...
        Quaternionf held=new Quaternionf(active.endVisual());
        Quaternionf result=new Quaternionf(active.startVisual()).slerp(held,GravityTransition.easeOutQuadratic(progress));
        if(progress>=1.0F){
            put(animation,new Active(active.entity(),actual,null,active.sequence(),held,null,active.receivedNanos(),0L,true,Mode.HOLD,0L));
            return held;
        }
        return result;
    }
    ...
    private static Quaternionf snapOverride(GravityRotationAnimation animation,Active active,Direction actual,long now){
        ...
        if(!active.targetSeen()){
            animation.forceSet(active.target(),now);
            active=new Active(active.entity(),active.target(),active.kind(),active.sequence(),active.startVisual(),active.endVisual(),active.receivedNanos(),now,true,active.mode(),active.durationNanos());
            put(animation,active);
        }
        long elapsed=Math.max(0L,now-active.startedNanos());
        long duration=active.durationNanos();
    ...
    public static boolean holding(Entity entity){if(!hasActiveTransitions)return false;GravityRotationAnimation animation=animation(entity);if(animation==null)return false;synchronized(ACTIVE){var active=ACTIVE.get(animation);return active!=null&&(active.mode()==Mode.HOLD||active.mode()==Mode.RECOVER_HOLD);}}
