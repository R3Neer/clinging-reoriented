...
/** Linkage-safe optional bridge: Clinging publishes semantic facts; Alchemical Leather owns costs and armor. */
public final class AlchemicalLeatherCompat implements ModInitializer {
    ...
    public static void successfulTurn(ServerPlayer player){
        ...
        Holder<MobEffect> effect=player.hasEffect(Reorientation.EFFECT)?Reorientation.EFFECT:clinging();
        if(effect!=null)publish(player,effect,TURN,1.0);
    }

    private static void controlledFlightTick(ServerPlayer player){
        if(emit==null||!player.hasEffect(Reorientation.EFFECT)||player.isPassenger())return;
        if(!ClingingReoriented.controlsPhysics(player)||AirChanges.grounded(player))return;
        var state=ClingingReoriented.data(player);
        if(state.groundedOnSurface)return;
        if(AnatomyBridge.active(player)&&AnatomyBridge.supported(player))return;
